	...
	if( f.name.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#grunfeld-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#grunfeld-contact').find('input,textarea').attr({value:''});
				jQuery('#grunfeld-contact').css({opacity:1});
				jQuery('#grunfeld-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#grunfeld-contact').animate({opacity:0.3});
		jQuery('#grunfeld-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#grunfeld-contact').find('input,textarea').css('border','none');
